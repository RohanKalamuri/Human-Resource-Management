...
class UserBase(MongoBaseModel):
    ...
    profile_picture: Optional[str] = Field(default=None, max_length=500, description="URL or path to the profile image.")
...
class UserSelfUpdate(BaseModel):
    """
    Payload for an employee updating their own profile via /api/users/me.
    Intentionally restricted to non-sensitive, self-serviceable fields.
    Role, email, employee_id, and active status can NOT be changed here.
    """
...
class UserAdminUpdate(BaseModel):
    """
    Payload for an admin/HR user overriding any field on another user's
    profile via /api/users/{user_id}. Every field is optional so callers
    can send a partial patch.
    """

    full_name: Optional[str] = Field(default=None, min_length=1, max_length=120)
    email: Optional[EmailStr] = None
    employee_id: Optional[str] = Field(default=None, min_length=1, max_length=32)
    department: Optional[str] = Field(default=None, max_length=80)
...
class AttendanceCheckInRequest(BaseModel):
    """
    Payload for POST /api/attendance/check-in.
    The server — never the client — stamps the actual check-in time, so
    only a couple of optional, non-time fields are accepted here.
    """

    work_from_home: bool = Field(default=False, description="Set true if checking in remotely.")
    notes: Optional[str] = Field(default=None, max_length=280)


class AttendanceCheckOutRequest(BaseModel):
    """Payload for POST /api/attendance/check-out."""
...
class AttendanceTodayOut(MongoBaseModel):
    """
    Row shape for the admin 'live today' tracker: attendance data enriched
    with just enough employee identity info to render a dashboard without
